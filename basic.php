...
<body>
  ...
  <template id="sidemenu.html">
    <ons-page>
      ...
      <ons-list>
        <ons-list-item onclick="fn.loadLink('https://esoftgreat.com')">
          <div class="left">
  ...
  <template id="home.html">
    <ons-page>
      <p class="intro">
        Selamat Datang di Aplikasi Monitoring KBM SMK Negeri 1 Bangsri.<br><br>
      </p>

      <ons-card onclick="fn.pushPage({'id': 'absensi.html', 'title': 'Absensi'})">
        <div class="title">Absensi</div>
        <div class="content">Menu Absensi.</div>
      </ons-card>
  ...
  <template id="absensi.html">
    <ons-page>
      ...
      <ons-list id="jurusan-list">

      </ons-list>
      <script>
        var getJurusan = function () {
          const names = ['RPL', 'BDP', 'AKL', 'TBSM', 'OTKP'];
          return names;
        };
        var makeJurusan = function () {
          const data = [];
          for (jurusan of getJurusan()) {
            data.push({ name: jurusan });
          }
          return data;
        }
        var createJurusan = function (jurusan) {
          return ons.createElement(`
                      <ons-list-item onclick="fn.pushPage({'id': 'kelas.html', 'title': '${jurusan.name}'}, 'lift-ios')">
                        <div class="left">
                          <img class="list-item__thumbnail" src="assets/images/class.webp">
                        </div>
                        <div class="center">${jurusan.name}</div>
                      </ons-list-item>
                    `
          );
        };
        var jurusan = makeJurusan();
        for (jurus of jurusan) {
          var opsijurusan = createJurusan(jurus);
          document.getElementById('jurusan-list').appendChild(opsijurusan);
        };

      </script>
    </ons-page>
  </template>
  <template id="kelas.html">
    <ons-page>
      <ons-toolbar>
        <div class="left">
          <ons-back-button>Jurusan</ons-back-button>
        </div>
      </ons-toolbar>
      <ons-list id="kelas-list">

      </ons-list>
      <script>
        var getKelas = function (title) {
          if (title == 'OTKP') {
            var names = ['X ' + title + ' 1', 'X ' + title + ' 2', 'X ' + title + ' 3', 'XI ' + title + ' 1', 'XI ' + title + ' 2', 'XI ' + title + ' 3', 'XI ' + title + ' 2', 'XII ' + title + ' 1', 'XII ' + title + ' 2', 'XII ' + title + ' 3'];
          } else {
            var names = ['X ' + title + ' 1', 'X ' + title + ' 2', 'XI ' + title + ' 1', 'XI ' + title + ' 2', 'XI ' + title + ' 2', 'XII ' + title + ' 1', 'XII ' + title + ' 2'];
          }

          return names;
        };
        var makeKelas = function (title) {
          const data = [];
          for (kelas of getKelas(title)) {
            data.push({ name: kelas });
          }
          return data;
        }
        var createKelas = function (kelas) {
          return ons.createElement(`
                <ons-list-item onclick="fn.pushPage({'id': 'siswa.html', 'title': '${kelas.name}'}, 'lift-ios')">
                  <div class="left">
                    <img class="list-item__thumbnail" src="assets/images/class.webp">
                  </div>
                  <div class="center">${kelas.name}</div>
                </ons-list-item>
              `
          );
        };
        ons.getScriptPage().onInit = function () {
          var kelas = makeKelas(this.data.title);
          for (kls of kelas) {
            var klas = createKelas(kls);
            document.getElementById('kelas-list').appendChild(klas);
          };
        }
  ...
  <template id="siswa.html">
    <ons-page>
      <ons-toolbar>
        <div class="left">
          <ons-back-button>Kelas</ons-back-button>
        </div>
      </ons-toolbar>
      <ons-list id="siswa-list">
      </ons-list>
      <script>
        async function getSiswa(title) {
          let url = _URL+'api/siswa.php?kelas=' + encodeURI(title);
          let obj = await (await fetch(url)).json();
          let el;
          let absensi = [];
          function switchAbsensi(event, elem, siswa_id) {
            var el = document.getElementById(elem);
            if (event.value) {
              el.innerHTML = `&nbsp;(Berangkat)`;
              absensi[siswa_id].status = 1;
            } else {
              el.innerHTML = `&nbsp;(Absen)`;
              absensi[siswa_id].status = 0;
            }
            console.log(absensi);
          }
          for (let siswa of obj) {
            absensi[siswa.id] = { siswa_id: siswa.id, status: 1 };
            el = ons.createElement(`
                  <ons-list-item>
                    <div class="left" onclick="app.showAction()">
                      <img class="list-item__thumbnail" src="assets/images/class.webp">
                    </div>
                    <div class="center">
                        ${siswa.nama} <span id="switch-status-${siswa.id}" input-id="${siswa.id}">&nbsp;(Berangkat)</span>
                    </div>
                    <div class="right">
                      <ons-switch id="model-switch-${siswa.id}" input-id="switch-${siswa.id}" checked="true"></ons-switch>
                    </div>
                  </ons-list-item>
                `
            );
            document.getElementById('siswa-list').appendChild(el);
            (function () {
              document.getElementById('model-switch-' + siswa.id).addEventListener('change', function (event) {
                var switchSiswa = 'switch-status-' + siswa.id;
                switchAbsensi(event, switchSiswa, siswa.id);
              }, false);
            }());
          }
          console.log(absensi);
        ...
        ons.getScriptPage().onInit = function () {
          getSiswa(this.data.title);
        }
        var app = {};
        app.showAction = function () {
          ons.openActionSheet({
            cancelable: true,
            title: 'Pilih Aksi',
            buttons: [
    ...
    <ons-page id="dialogs-page">
      ...
      <ons-action-sheet id="action-sheet-dialog" cancelable>
        ...
        <ons-action-sheet-button onclick="fn.hideDialog('action-sheet-dialog')" modifier="destructive"
          icon="md-square-o">
          Action 3</ons-action-sheet-button>
  ...
  <template id="loadMore.html">
    ...
    <ons-page id="load-more-page" on-infinite-scroll="fn.loadMore">
      <p class="intro">
        Useful for loading more items when the scroll reaches the bottom of the page, typically after an asynchronous
        API
        call.<br><br>
      </p>
  ...
  <template id="lazyRepeat.html">
    ...
    <ons-page id="lazy-repeat-page">
      <p class="intro">
        Automatically unloads items that are not visible and loads new ones. Useful when the list contains thousands of
        items.<br><br>
      </p>
